...
description: >-
  ...
  iphone with blip", "blip this to nick". Resolves a friendly device or contact
  name to a Blip peer id and sends through the genuine Blip app (no UI driving).
when_to_use: >-
  The user wants to send/share a local file to one of their own devices or a known
  contact via Blip ("blip this to my mac", "send X to my iPhone with blip").
---

# Blip a file to a device or contact

Sends file(s) to one of the user's Blip-paired devices (Mac, iPhone, iPad, PC,
…) or to a known contact, using the `blip-send` CLI. Blip must be installed and
signed in, and ideally already running.

## Prerequisite

The `blip-send` CLI must be on PATH:

```
pipx install git+https://github.com/BenjaminHolderbein/blip-cli
```

Verify with `blip-send doctor`. (If it isn't installed, install it, or run the
checked-out launcher instead: `python blip_send.py …` on Windows, `python3
blip_send.py …` on macOS/Linux.)
...
3. **Send**, passing the device or contact name to `--to` (partial,
   ...
   ```
   blip-send send --to "MacBook Pro" "/path/to/file"
   ```
...
- If the CLI can't find Blip's state or transport, run `blip-send doctor` and
